...
// ─── Proxy bootstrap ────────────────────────────────────────────────
// Node.js native fetch() does NOT read HTTP_PROXY / HTTPS_PROXY.
// Use undici's ProxyAgent to intercept all fetch() calls.
const PROXY_URL =
	process.env.PI_PROXY_URL ||
	process.env.NODE_PROXY_URL ||
...
if (PROXY_URL) {
	...
	try {
		// Try node_modules relative to this script first, then global
		const { ProxyAgent, setGlobalDispatcher } = await import(
			new URL("../node_modules/undici/index.js", import.meta.url).href
		).catch(() => import("undici"));
		setGlobalDispatcher(new ProxyAgent(proxyUrl));
		console.error(`[web-search] Proxy enabled: ${proxyUrl}`);
	} catch {
		console.error(
			`[web-search] undici not found, proxy NOT enabled (set HTTP_PROXY)`,
		);
	}
}
...
function collectModuleCandidates(
	...
	for (const start of [cwd, scriptDir]) {
		...
		for (let i = 0; i < 8; i++) {
			add(
				join(dir, "node_modules", "@mariozechner", "pi-ai", "dist", fileName),
			);
...
async function resolveApiKey(provider, auth, authPath, piAi) {
	...
	if (typeof oauth.getOAuthApiKey !== "function") {
		...
		throw new Error(
			oauth.error || "Loaded pi-ai module does not export getOAuthApiKey",
		);
	}
